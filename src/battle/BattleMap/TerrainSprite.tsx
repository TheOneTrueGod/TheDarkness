...
import { extractTilesetImage } from "../Managers/TilesetHelpers";
...
export default class TerrainSprite {
  ...
  constructor(tileCoord: TileCoord, pixiLoader: PIXI.Loader) {
    ...
    this.createTerrainSprite(pixiLoader, tileCoord.x * 546 + tileCoord.y * 837);

    this.pixiLoader = pixiLoader;
    // @ts-ignore
    this.terrainEffects = Object.keys(TerrainEffects).map(
  ...
  showTerrainEffect(terrainEffect: TerrainEffects) {
    if (this.terrainEffects[terrainEffect] === undefined) {
      this.terrainEffects[terrainEffect] =
        this.getTerrainEffectSprite(terrainEffect);
    }
    ...
    switch (terrainEffect) {
      case TerrainEffects.TARGETTER_ATTACK:
        sprite = new PIXI.Sprite(
          this.pixiLoader.resources[SpriteList.CROSSHAIR].texture
        );
      case TerrainEffects.TARGETTER_MOVE:
        sprite = new PIXI.Sprite(
          this.pixiLoader.resources[SpriteList.POSITION_MARKER].texture
        );
    }
  ...
  createTerrainSprite(pixiLoader: PIXI.Loader, variantSeed: number) {
    const { x: tileSizeX, y: tileSizeY } = getTileSize();

    const terrainSprite = extractTilesetImage(
      pixiLoader.resources[SpriteList.TILESET].texture,
      "EMPTY_SPACE",
      variantSeed
    );
