...
export default class AbilityKineticMeleeRelease extends BaseAbility {
    ...
    doesUnitHaveResourcesForAbility(unit: BattleUnit) {
        return unit.hasResource(UnitResourceTypes.KINETIC_ENERGY, this.energyCost) && super.doesUnitHaveResourcesForAbility(unit);
    }
    ...
    playOutAbility(gameDataManager: GameDataManager, unit: BattleUnit, targets: Array<AbilityTarget>, doneCallback: Function) {
        ...
        for (let xOff = -1; xOff < unitSize.x + 1; xOff += 1) {
            for (let yOff = -1; yOff < unitSize.y + 1; yOff += 1) {
                ...
                if (!xOnEdge && !yOnEdge) { continue; }

                const targetCoord = { x: unit.tileCoord.x + xOff, y: unit.tileCoord.y + yOff };
                const targetUnit = gameDataManager.unitManager.getUnitAtTileCoord(targetCoord, gameDataManager.clientBattleMap);
