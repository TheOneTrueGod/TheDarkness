...
class MissionsEndpoint {
  static getResponse(user, uri, request, body) {
    if (uri.startsWith("/api/create-mission")) {
      ...
      if (!campaign) {
        throw new Error(`Couldn't find campaign ${body.campaignId}`);
      }
    ...
    if (uri.startsWith("/api/missions/remove-unit")) {
      return this.removeUnitFromMission(
        body.campaignId,
        body.missionId,
        body.unitId
      );
  ...
  static removeUnitFromMission(campaignId, missionId, unitId) {
    ...
    const missionUnit = mission.caravan.unitList.find(
      (missionUnit) => missionUnit.unitId === unitId
    );
    if (!missionUnit) {
      throw new Error(`Unit not in mission (unit id ${unitId})`);
    }

    mission.caravan.unitList.splice(
      mission.caravan.unitList.indexOf(missionUnit),
      1
    );
    saveMission(mission);
  ...
  static addUnitToMission(campaignId, missionId, unitId) {
    ...
    const campaignUnit = campaign.campaignUnits.find(
      (campaignUnit) => campaignUnit.unitId === unitId
    );
  ...
  static getMission(campaignId, missionId, includeAllEvents) {
    ...
    throw new Error(
      `Mission '${missionId}' from campaign '${campaignId}' not found!`
    );
  }
}

export { MissionsEndpoint };
